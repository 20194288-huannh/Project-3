  ...
  <b-container>
    ...
    <b-row>
      <b-col style="flex: 20">
        ...
        <div>
          Price range: ₹{{ search.min_price }} - ₹{{ search.max_price }}
        </div>
        <multi-range-slider
          class="pt-3"
          baseClassName="multi-range-slider-bar-only"
          :minValue="search.min_price"
          :maxValue="search.max_price"
          :max="50000"
          :min="0"
          :step="100"
          :rangeMargin="0"
        ...
        <div>
          Discount Range: {{ search.min_discount }}%-{{ search.max_discount }}%
        </div>
        <multi-range-slider
          class="pt-3"
          baseClassName="multi-range-slider-bar-only"
          :minValue="search.min_discount"
          :maxValue="search.max_discount"
          :max="50"
          :min="0"
          :step="1"
          :rangeMargin="0"
      ...
      <b-col style="flex: 80">
        ...
        <div class="grid-container">
          <VerticalBook
            ...
            :key="book.id"
            :book="book"
            @show-modal-quick-view="showQuickView"
          />
...
export default {
  ...
  methods: {
    ...
    infiniteHandler($state) {
      // gọi API để lấy danh sách items mới
      this.GET(`${this.api}?page=${this.page}`, this.search).then(response => {
        const newItems = response.data.data
        // thêm items mới vào danh sách hiện có
        this.books = [...this.books, ...newItems]
        // tăng số trang lên 1
        this.page++
        // thông báo cho InfiniteLoading component biết kết quả xử lý
        $state.loaded()
      })
    }
  },
...
<style
  src="../../node_modules/multi-range-slider-vue/MultiRangeSliderBarOnly.css"
></style>
