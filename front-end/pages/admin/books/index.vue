<template>
  <div class="my-5">
    <b-form inline>
      ...
      />
      <b-form-select
        v-if="search.category_level_2"
        v-model="search.category_level_3"
        class="mb-2 mx-sm-2 mb-sm-0"
        :options="categories_level_2"
        :value="null"
        @change="category_level_3()"
      />
    </b-form>
    ...
    async fetchBooks() {
      try {
        const response = await this.$axios.get("/products", {
          params: {
            page: this.page,
            category: this.search.category_level_3 || this.search.category_level_2 || this.search.category_level_1,
          },
        });
